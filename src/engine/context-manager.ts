...
export class ContextManager {
  constructor(private def: WorkflowDefinition, private policy: ContextPolicy = DEFAULT_CONTEXT_POLICY) {}
  ...
  buildInputContext(node: AgentNode, state: ExecutionState): InputContext {
    const contract: InputContract = node.inputContract;
    ...
    const typeFiltered = selected.filter((edge) => {
      if (!contract.acceptedTypes?.length) return true;
      const upstream = this.def.nodes.find(n => n.id === edge.source.nodeId);
      if (!upstream) return true;
      const produced: ArtifactKind[] = upstream.outputContract.artifactTypes?.length
    ...
    const items: (InputContextItem & { edge: WorkflowEdge; priority: number })[] = [];
    ...
    for (const edge of typeFiltered) {
      ...
      if (!latest) continue; // 上游尚无输出（被 skip 等）
