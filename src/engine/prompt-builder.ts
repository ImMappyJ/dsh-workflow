...
export function buildPrompt(node: AgentNode, ctx: InputContext): CompiledPrompt {
  ...
  const routingOpts = node.metadata['__routingOptions'] as string | undefined;
  ...
  const inputSection = ctx.inputs.length === 0
    ? '（无上游输入）'
    : ctx.inputs.map(i =>
        `## 来自 ${i.sourceName}（第 ${i.runIndex} 轮）\n${i.content}`,
      ).join('\n\n');

  // 需求 3：本地工作目录注入（engine 经 metadata 传入，不污染节点配置）
  const wsDir = (node.metadata['__workspaceDir'] as string | undefined) ?? '';
  ...
  const execution = [
    ...
    (node.metadata['__iterationHint'] as string | undefined) ?? '',
    wsSection,
  ].filter(Boolean).join('\n');
