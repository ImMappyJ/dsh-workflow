...
export class NodeScheduler {
  private loopOfNode = new Map<string, string>();
  ...
  private upstreamIds(nodeId: string): string[] {
   ...
   */
  private resolveThroughDisabled(ids: string[]): string[] {
    const queue = [...ids];
    ...
    while (queue.length) {
      ...
      if (this.disabled.has(u)) {
        for (const uu of this.upstreamIds(u)) {
          if (!seen.has(uu)) { seen.add(uu); queue.push(uu); }
        }
      } else {
        out.push(u);
      }
   ...
   */
  private loopAwaitingReview(nodeId: string, state: ExecutionState): boolean {
    const myLoop = this.loopOfNode.get(nodeId);
    if (myLoop === undefined) return false;
    // 仅阻塞有 pending review 的节点自身，不阻塞同环其他成员
    return state.reviewTasks.some(t =>
      t.status === 'pending' && t.sourceNodeId === nodeId);
  }

  /** 必需依赖全部有输出 */
  depsSatisfied(nodeId: string, state: ExecutionState): boolean {
    if (this.upstreamIds(nodeId).length === 0) return false;   // 无入边（只有 start 合法）
  ...
  wouldRun(nodeId: string, state: ExecutionState): boolean {
    if (this.disabled.has(nodeId)) return false;   // 禁用节点永不参与调度（任务 2）
    if (!this.depsSatisfied(nodeId, state)) return false;
    if (!this.edgesPassable(nodeId, state)) return false;   // Review Gate 阻塞（§58）
    if (this.loopAwaitingReview(nodeId, state)) return false; // §69 审核期冻结：等待人工评估
    ...
    if (ns.status === 'skipped' || ns.status === 'cancelled') return false;
