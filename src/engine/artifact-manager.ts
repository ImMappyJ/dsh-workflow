...
import type {
  Artifact, ArtifactKind, ArtifactStorageRef, CodeFileEntry, ExecutionState, NodeId,
} from '../domain/types.js';
import { kindFromLegacyType, kindFromFormat, legacyTypeFromKind } from '../domain/types.js';
import { lineDiff, type DiffResult } from './diff.js';
...
// ============================================================
// ArtifactManager（第二阶段：类化，支持磁盘引用与版本操作）
// ============================================================
...
export class ArtifactManager {
  ...
  private artifactDir(executionId: string, artifactId: string): string {
    return path.join(this.dataDir, 'artifacts', executionId, artifactId);
  }

  private ext(kind: ArtifactKind): string {
    switch (kind) {
  ...
  async read(artifact: Artifact): Promise<string> {
    if (artifact.storage !== 'reference' || !artifact.storageRef) return artifact.content;
    if (artifact.storageRef.kind === 'directory') {
      // 目录：返回文件清单摘要
      try {
        const entries = await fs.readdir(artifact.storageRef.path, { recursive: true } as never);
        return `[directory:${artifact.storageRef.path}]\n${(entries as string[]).slice(0, 200).join('\n')}`;
      } catch {
  ...
  async createCode(params: {
    ...
    for (const f of params.files ?? []) {
      ...
      const entry: CodeFileEntry = { path: rel, content: f.content ?? '', language: f.language };
      if (bytes > this.limit) {
  ...
  async readCodeFile(artifact: Artifact, filePath: string): Promise<string> {
    const rel = sanitizeRelPath(filePath);
    const entry = artifact.files?.find(f => f.path === rel);
    if (!entry) throw new Error(`文件不存在：${rel}`);
    if (entry.storageRef) return fs.readFile(entry.storageRef.path, 'utf8');
   ...
   */
  async diffCode(a: Artifact, b: Artifact): Promise<FileDiffEntry[]> {
  ...
  /** 版本对比（diff） */
  async diff(a: Artifact, b: Artifact): Promise<DiffResult> {
   ...
   */
  async restore(state: ExecutionState, nodeId: NodeId, targetVersion: number): Promise<Artifact> {
    const chain = state.artifacts[nodeId] ?? [];
    const target = chain.find(a => a.version === targetVersion);
    if (!target) throw new Error(`节点 ${nodeId} 不存在版本 v${targetVersion}`);
    const prev = chain.at(-1);
    if (!prev) throw new Error(`节点 ${nodeId} 无版本链`);
...
function registerArtifact(state: ExecutionState, artifact: Artifact): void {
 ...
 */
export function humanEditArtifact(
  state: ExecutionState,
  nodeId: NodeId,
  content: string,
  ...
  if (!prev) throw new Error(`节点 ${nodeId} 尚无 Agent 产出，无法编辑`);
  const artifact: Artifact = {
    id: newId(),
    type: prev.type,
...
/** 当前批准版本（最新版本） */
export function approvedArtifactOf(state: ExecutionState, nodeId: NodeId): Artifact | null {
  return state.artifacts[nodeId]?.at(-1) ?? null;
}

/** 版本链（含 Agent 原始与历次人工修改） */
export function artifactVersions(state: ExecutionState, nodeId: NodeId): Artifact[] {
  return state.artifacts[nodeId] ?? [];
}
