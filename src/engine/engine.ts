...
interface ControlFlags {
  ...
  stepWaiter: (() => void) | null;
  reviewWaiter: (() => void) | null;
  /** Human Task 提交唤醒器（Phase 11） */
  humanWaiter: (() => void) | null;
  /** 审核超时唤醒定时器（§70） */
  reviewTimer: ReturnType<typeof setTimeout> | null;
  /** 已处理过超时策略的 ReviewTask（pause 策略防重复处理） */
  handledReviewTimeouts: Set<string>;
}
...
export class WorkflowEngine {
  ...
  private controls = new Map<string, ControlFlags>();
  private states = new Map<string, ExecutionState>();
  /** 每个执行的环控制器（resolveReview 需要判定源节点所属环以设置收敛冻结，§69） */
  private loopers = new Map<string, LoopController>();
   ...
   */
  resolveReview(executionId: string, taskId: string, action: 'accept' | 'reject' | 'edit' | 'accept_after_edit' | 'terminate',
    opts: { comment?: string | null; content?: string; operator?: 'human' | 'system' } = {}) {
    const state = this.states.get(executionId);
    if (!state) throw new Error(`execution ${executionId} 不存在（可能已结束）`);
    const outcome = applyReviewDecision(state, taskId, action, opts);
    this.emit(executionId, 'review.resolved',
      { action, taskId, status: outcome.task.status, comment: opts.comment ?? null },
      outcome.task.sourceNodeId, outcome.task.edgeId);
    ...
    if (outcome.terminate) {
      ...
      if (ctrl) ctrl.stopRequested = true;   // 主循环顶部短路返回，保留 terminated 状态
    }
    ...
    if (!outcome.terminate) {
      ...
      const isNodeReview = outcome.task.edgeId === '__node__' as any;
    ...
    if (ctrl?.reviewWaiter) { ctrl.reviewWaiter(); ctrl.reviewWaiter = null; }
    return outcome;
  }
   ...
   */
  resolveAllReviews(executionId: string, action: 'accept' | 'reject', opts: { comment?: string | null } = {}): number {
    const state = this.states.get(executionId);
    if (!state) throw new Error(`execution ${executionId} 不存在（可能已结束）`);
    if (action === 'reject' && !opts.comment) throw new Error('Reject All 必须提供统一审核意见（§61）');
    const pending = state.reviewTasks.filter(t => t.status === 'pending').map(t => t.id);
    for (const taskId of pending) {
      try { this.resolveReview(executionId, taskId, action, { comment: opts.comment ?? null }); }
   ...
   */
  private createHumanTask(def: WorkflowDefinition, state: ExecutionState, node: AgentNode, scheduler: NodeScheduler) {
    ...
    const task: HumanTask = {
      ...
      nodeId: node.id,
      prompt: (node.metadata?.['taskPrompt'] as string) || node.roleDescription || '请完成此人工任务并提交结果。',
      status: 'pending',
      content: null,
      note: null,
   ...
   */
  submitHumanTask(executionId: string, taskId: string, opts: { content: string; note?: string; operator?: string }) {
    const state = this.states.get(executionId);
    if (!state) throw new Error(`execution ${executionId} 不存在（可能已结束）`);
    const task = state.humanTasks?.find(t => t.id === taskId);
    if (!task) throw new Error(`human task ${taskId} 不存在`);
    if (task.status !== 'pending') throw new Error(`human task ${taskId} 已处理（${task.status}）`);
    ...
    const format: Artifact['type'] = node?.outputContract.format === 'json' ? 'json'
   ...
   */
  private async spillOversize(state: ExecutionState, artifact: Artifact, content: string): Promise<void> {
    if (Buffer.byteLength(content, 'utf8') <= INLINE_LIMIT) return;
    ...
    if (created.storage !== 'reference' || !created.storageRef) return;
   ...
   */
  async restoreArtifact(executionId: string, nodeId: string, targetVersion: number): Promise<Artifact> {
    const state = this.states.get(executionId);
    if (!state) throw new Error(`execution ${executionId} 不存在（可能已结束）`);
  ...
  async editCodeArtifact(
    ...
    if (!prev) throw new Error(`节点 ${nodeId} 无 code Artifact，无法编辑`);
    // 合并语义：提交可只带被修改的文件；content 为 null/空串视为"未修改"，从上一版本回填（大文件前端无内存内容）。
    const submitted = new Map(files.map(f => [f.path, f]));
    const merged: Array<{ path: string; content: string; language?: string }> = [];
    ...
    for (const sub of submitted.values()) {
      if (sub.content != null) merged.push({ path: sub.path, content: sub.content, language: sub.language });
    }
  ...
  control(executionId: string, cmd: 'pause' | 'resume' | 'stop' | 'step'): void {
    ...
    if (cmd === 'resume') {
      c.paused = false; c.stepMode = false;
      const st = this.states.get(executionId);
      if (st && st.status === 'paused') st.status = 'running';
      this.emit(executionId, 'execution.resumed');
      c.resumeWaiter?.(); c.resumeWaiter = null;
    }
    if (cmd === 'stop') {
      c.stopRequested = true;
      c.resumeWaiter?.(); c.stepWaiter?.();
      c.reviewWaiter?.(); c.reviewWaiter = null;
      c.humanWaiter?.(); c.humanWaiter = null;
    }
    if (cmd === 'step') {
      c.stepMode = true;
      c.stepWaiter?.(); c.stepWaiter = null;   // 放行一步
    }
  }

  async run(def: WorkflowDefinition, input: RunInput): Promise<ExecutionHandle> {
    // 运行前校验并补全 LoopConfig
    ...
    const effectiveDef: WorkflowDefinition = { ...def, loops };
    ...
    const state: ExecutionState = {
      ...
      defSnapshot: structuredClone(effectiveDef) as WorkflowDefinition,
   ...
   */
  private resolveArtifactRef(state: ExecutionState, ref: string): { nodeId: string; artifact: import('../domain/types.js').Artifact } | null {
    const m = ref.match(/^(.*)@v(\d+)$/);
    if (m) {
      const nodeId = m[1]; const ver = Number(m[2]);
    ...
    for (const [nodeId, chain] of Object.entries(state.artifacts ?? {})) {
      const art = [...chain].reverse().find(a =>
        a.id === ref || a.storageRef?.fileName === ref ||
        (Array.isArray(a.files) && a.files.some(f => f.path === ref)));
      if (art) return { nodeId, artifact: art };
    }
    return null;
  }
   ...
   */
  async rework(def: WorkflowDefinition, parentState: ExecutionState, reworkNodeId: NodeId, input: RunInput): Promise<ExecutionHandle> {
    const node = def.nodes.find(n => n.id === reworkNodeId);
    if (!node) throw new Error(`rework 节点 ${reworkNodeId} 不存在于当前定义`);
    if (node.type === 'start' || node.type === 'end') throw new Error('start/end 不可作为 Rework 起点');
    const validation = this.validate(def);
    if (!validation.valid) {
      throw new Error(`workflow invalid: ${validation.errors.map(e => e.message).join('; ')}`);
    }
    ...
    const rerun = new Set<NodeId>([reworkNodeId]);
    const queue: NodeId[] = [reworkNodeId];
    while (queue.length) {
      const cur = queue.shift()!;
      for (const e of effectiveDef.edges) {
        if (e.source.nodeId === cur && !rerun.has(e.target.nodeId)) { rerun.add(e.target.nodeId); queue.push(e.target.nodeId); }
      }
    }
    for (const n of effectiveDef.nodes) {
      if ((n.type === 'start' || n.type === 'end') && n.id !== reworkNodeId) rerun.delete(n.id);
    }

    // 基底 = 父状态深拷贝（继承但不污染，原则 17）；关键身份字段换成新执行
    const inherited = JSON.parse(JSON.stringify(parentState)) as ExecutionState;
    const state: ExecutionState = {
      ...inherited,
      executionId,
      ...
      nodeStates: Object.fromEntries(effectiveDef.nodes.map(n => {
        if (n.id === reworkNodeId) return [n.id, { status: 'idle' as const, iteration: 0, inputReady: [], attempt: 0 }];
        if (rerun.has(n.id)) {
          // 冻结在继承输入版本（inputs 来自父执行继承的 outputs）
          return [n.id, { status: 'success' as const, iteration: 0, inputReady: [], attempt: 0, lastInputVersion: scheduler.inputVersion(n.id, inherited as ExecutionState) }] ;
        }
        return [n.id, { status: 'success' as const, iteration: 0, inputReady: [], attempt: 0, lastInputVersion: Number.MAX_SAFE_INTEGER }];
      })),
      reviewTasks: [],
    ...
    if (input.modifiedArtifacts?.length) {
      for (const ref of input.modifiedArtifacts) {
        ...
        if (outs?.length) outs[outs.length - 1] = { ...outs[outs.length - 1], content: hit.artifact.content };
      }
    }
    this.states.set(executionId, state);
    const ctrl: ControlFlags = {
  ...
  private async executeLoop(
    ...
    while (true) {
      ...
      if (ctrl.paused) {
        await new Promise<void>(r => { ctrl.resumeWaiter = r; });
        continue;
      }
      if (ctrl.stepMode) {
        ctrl.stepMode = false;
        await new Promise<void>(r => { ctrl.stepWaiter = r; });
        if (ctrl.paused || ctrl.stepMode) continue;
      }
      ...
      if (runnable.length === 0) {
        ...
        if (scheduler.hasPendingReview(state)) {
          if (this.handleTimedOutReviews(def, state, ctrl)) continue;
          ...
          await new Promise<void>(r => {
            ...
            if (delay !== null) {
              ctrl.reviewTimer = setTimeout(() => {
                ...
                ctrl.reviewWaiter?.(); ctrl.reviewWaiter = null;
              }, delay);
            }
          });
          if (ctrl.reviewTimer) { clearTimeout(ctrl.reviewTimer); ctrl.reviewTimer = null; }
          // 等待期间可能被 resolveReview 改为 terminated；其他情况恢复 running（TS 无法感知异步修改）
          if ((state.status as string) !== 'terminated') state.status = 'running';
        ...
        if (scheduler.hasPendingHumanTask(state)) {
          ...
          await new Promise<void>(r => { ctrl.humanWaiter = r; });
          if ((state.status as string) !== 'terminated') state.status = 'running';
          continue;
        }
   ...
   */
  private limitTerminationCheck(def: WorkflowDefinition, state: ExecutionState, looper: LoopController, scheduler: NodeScheduler): 'loop' | 'node' | false {
    ...
    if (!decisions.every(d => !d.allowed)) return false;
  ...
  private async runNode(
    ...
    if (def.settings?.workspaceDir) node.metadata['__workspaceDir'] = def.settings.workspaceDir;
    ...
    if (feedback) {
      ...
      if (attached.length) {
        prompt.user += '\n\n# HUMAN ATTACHED INPUT\n' + attached.map(a =>
          `[来源 ${a!.nodeId} @v${a!.artifact.version}]\n${a!.artifact.content}`).join('\n---\n');
      }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      if (ctrl.stopRequested) { ns.status = 'cancelled'; return; }
      ns.attempt = attempt;
      const started = Date.now();
      try {
        let content = '';
        let record: Omit<import('../domain/types.js').NodeOutputRecord, 'runIndex'> | null = null;
        // §4：节点持久化绑定 DSH Conversation——优先复用已有 sessionId
        const existingSessionId = node.metadata['conversationId'] as string | undefined;
        ...
        let gated = false;
        for (const e of def.edges.filter(e => e.source.nodeId === nodeId)) {
          if (state.edgeState[e.id]?.passable === false) continue;   // routing 已拒绝的边不再触发审核
  ...
  private markUnfinished(state: ExecutionState, status: 'skipped' | 'cancelled') {
    for (const ns of Object.values(state.nodeStates)) {
      if (['idle', 'waiting', 'queued', 'running', 'waiting_review', 'waiting_human'].includes(ns.status)) ns.status = status;
    }
  ...
  private handleTimedOutReviews(def: WorkflowDefinition, state: ExecutionState, ctrl: ControlFlags): boolean {
    const nowMs = Date.now();
    const timedOut = state.reviewTasks.filter(t =>
      t.status === 'pending' && t.timeoutAt
  ...
  private nearestReviewTimeout(state: ExecutionState, ctrl: ControlFlags): number | null {
    const nowMs = Date.now();
    let min: number | null = null;
    for (const t of state.reviewTasks) {
      if (t.status !== 'pending' || !t.timeoutAt || ctrl.handledReviewTimeouts.has(t.id)) continue;
  ...
  private emit(executionId: string, type: ExecutionEvent['type'], payload?: unknown, nodeId?: NodeId, edgeId?: string) {
    const state = this.states.get(executionId);
 ...
 */
export function composeStartContent(def: WorkflowDefinition, input: RunInput): string {
  const fields = input.fields ?? {};
  const defs = def.inputSchema?.fields ?? [];
  if (!defs.length) return input.text;
  ...
  for (const f of defs) {
    ...
    switch (f.type) {
      ...
      case 'directory':
        ...
        break;
      case 'files':
        fieldLines.push(`${label}: ${(v as string[]).map(p => `[file:${p}]`).join(', ')}`);
        break;
      case 'artifact':
 ...
 */
function extractRouteFromOutput(content: string, format: string): string | undefined {
  if (format === 'json') {
    try {
      const parsed = JSON.parse(content);
      if (typeof parsed.route === 'string') return parsed.route;
      // 兼容嵌套结构：result.route
      if (parsed.result && typeof parsed.result.route === 'string') return parsed.result.route;
