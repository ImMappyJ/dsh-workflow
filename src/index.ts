...
import { DshSessionProvider, type ApiProxyLike } from './provider/dsh-session-provider.js';
import { createStorage, type PluginStorage } from './storage/repo.js';
import { loopsFromSccs } from './graph/validator.js';
import { BUILTIN_TEMPLATES, instantiateTemplate, type WorkflowTemplate } from './templates/builtin.js';
import type { ExecutionEvent, WorkflowDefinition } from './domain/types.js';
...
export function createWorkflowServer(opts: {
  ...
  const frameLog: Array<{ t: string; type: string; event?: any }> = [];
  ...
  const storage: PluginStorage = createStorage(opts.dataDir);
  ...
  const engine = new WorkflowEngine({
    ...
    onExecutionEnd: (state) => storage.executions.save({ ...state, id: state.executionId } as never).catch(() => {}),
  });

  // 活跃 execution 的状态引用
  const liveStates = new Map<string, () => void /* unsubscribe */>();
  ...
  async function api(req: IncomingMessage, res: ServerResponse, url: URL): Promise<void> {
    ...
    if (p === '/api/models' && opts.apiProxy) {
      const r: any = await (opts.apiProxy as any).llm?.models?.({ rpcId: randomUUID(), payload: {} });
    ...
    if (p === '/api/pick-directory' && method === 'POST') {
      ...
      if (opts.apiProxy && (opts.apiProxy as any).host?.pickDirectory) {
        try {
          const r: any = await (opts.apiProxy as any).host.pickDirectory({ rpcId: randomUUID(), payload: {} });
          if (!r?.error) {
            const p = r?.result?.value?.path ?? null;
            if (p) return json(res, 200, { ok: true, path: String(p).replace(/\\/g, '/') });
      ...
      if (!process.env.VITEST) {
        ...
        try {
          ...
          if (p) return json(res, 200, { ok: true, path: p });
    ...
    if (p === '/api/presets' && method === 'POST') {
      const body = await readBody<{ node?: any; name?: string; description?: string }>(req);
    ...
    if ((m = p.match(/^\/api\/presets\/([^/]+)$/)) && method === 'GET') {
      ...
      if (!pr) return json(res, 404, { error: '预设不存在' });
    ...
    if ((m = p.match(/^\/api\/templates\/([^/]+)$/)) && method === 'GET') {
      const tpl = BUILTIN_TEMPLATES.find(t => t.id === m![1]) ?? (await storage.templates.get(m[1])) as WorkflowTemplate | null;
      if (!tpl) return json(res, 404, { error: 'template not found' });
      return json(res, 200, tpl);
    }
    if (p === '/api/templates' && method === 'POST') {
      const body = await readBody<{ def?: WorkflowDefinition; name?: string; description?: string }>(req);
      if (!body.def?.id) return json(res, 400, { error: '缺少 def（完整工作流定义）' });
      const validation = engine.validate(body.def);
      if (!validation.valid) return json(res, 400, { error: 'validation failed', issues: validation.errors });
      const now = new Date().toISOString();
    ...
    if ((m = p.match(/^\/api\/templates\/([^/]+)$/)) && method === 'DELETE') {
      if (BUILTIN_TEMPLATES.some(t => t.id === m![1])) return json(res, 400, { error: '内置模板不可删除' });
    ...
    if ((m = p.match(/^\/api\/templates\/([^/]+)\/instantiate$/)) && method === 'POST') {
      ...
      const tpl = BUILTIN_TEMPLATES.find(t => t.id === tplId) ?? (await storage.templates.get(tplId)) as WorkflowTemplate | null;
      if (!tpl) return json(res, 404, { error: 'template not found' });
      const newId = body.id?.trim() || `${tplId.replace(/^tpl_/, 'wf_')}_${randomUUID().slice(0, 6)}`;
      if (await storage.workflows.get(newId)) return json(res, 400, { error: `工作流 ${newId} 已存在` });
    ...
    if (p === '/api/workflows/validate' && method === 'POST') {
      const def = await readBody<WorkflowDefinition>(req);
      if (!def?.id) return json(res, 400, { error: '缺少 id' });
      const validation = engine.validate(def);
      return json(res, 200, {
    ...
    if (p === '/api/workflows/import' && method === 'POST') {
      const def = await readBody<WorkflowDefinition>(req);
      if (!def?.id || !def?.nodes) return json(res, 400, { error: '无效的工作流定义：缺少 id 或 nodes' });
      const validation = engine.validate(def);
      if (!validation.valid) return json(res, 400, { error: '导入定义校验失败', issues: validation.errors });
      def.loops = loopsFromSccs(validation.loops, def.loops);
      def.updatedAt = new Date().toISOString();
    ...
    if ((m = p.match(/^\/api\/workflows\/([^/]+)$/))) {
      ...
      if (method === 'DELETE') {
        ...
        const memExecs = [...((engine as any).states as Map<string, any>).values()];
        ...
        for (const eid of execIds) {
          const st = (engine as any).states.get(eid);
          ...
          (engine as any).states.delete(eid);
        }
    ...
    if ((m = p.match(/^\/api\/workflows\/([^/]+)\/rework$/)) && method === 'POST') {
      ...
      const body = await readBody<{
        parentExecutionId?: string; reworkNodeId?: string;
        instruction?: string; input?: string; fields?: Record<string, unknown>;
        inputArtifacts?: string[]; modifiedArtifacts?: string[];
      }>(req);
      ...
      const parentState = ((engine as any).states as Map<string, any>).get(body.parentExecutionId)
        ?? await storage.executions.get(body.parentExecutionId);
      if (!parentState) return json(res, 404, { error: 'parent execution not found' });
      try {
        const handle = await engine.rework(def, parentState as never, body.reworkNodeId, {
    ...
    if ((m = p.match(/^\/api\/workflows\/([^/]+)\/run$/)) && method === 'POST') {
      const body = await readBody<{ input?: string; fields?: Record<string, unknown>; version?: number | string; inputArtifacts?: string[]; modifiedArtifacts?: string[] }>(req);
      ...
      try {
        ...
        const unsub = engine.subscribe(handle.executionId, () => { /* 事件仅用于 SSE/内存态 */ });
            ...
            for (const n of def.nodes) {
              if (n.metadata?.['conversationId']) {
                ...
                for (const [k, v] of Object.entries(n.metadata)) {
                  if (!k.startsWith('__')) cleanMeta[k] = v;
                }
    ...
    if (p === '/api/executions' && method === 'GET') {
      ...
      const mem = [...((engine as any).states as Map<string, any>).values()];
      const byId = new Map<string, any>();
      for (const d of disk) byId.set(d.executionId ?? d.id, d);
      for (const s of mem) byId.set(s.executionId, s); // 内存（活跃/最新）优先
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)$/)) && method === 'GET') {
      const id = m[1];
      const state = (engine as any).states?.get?.(id);
      if (state) {
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)$/)) && method === 'DELETE') {
      ...
      const memStates = [...((engine as any).states as Map<string, any>).values()];
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/control$/)) && method === 'POST') {
      const body = await readBody<{ cmd: string }>(req);
      const cmds = ['pause', 'resume', 'stop', 'step'];
      if (!cmds.includes(body.cmd)) return json(res, 400, { error: `cmd 必须是 ${cmds.join('/')}` });
      engine.control(m[1], body.cmd as never);
      return json(res, 200, { ok: true });
    }
    // 人工审核决策（§58-73）：accept / reject / edit / accept_after_edit / terminate
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/review$/)) && method === 'POST') {
      const body = await readBody<{ taskId?: string; action?: string; comment?: string; content?: string }>(req);
      const actions = ['accept', 'reject', 'edit', 'accept_after_edit', 'terminate'];
      if (!body.taskId) return json(res, 400, { error: 'taskId 必填' });
      if (!body.action || !actions.includes(body.action)) return json(res, 400, { error: `action 必须是 ${actions.join('/')}` });
      try {
        const outcome = engine.resolveReview(m[1], body.taskId, body.action as never, {
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/review-all$/)) && method === 'POST') {
      const body = await readBody<{ action?: string; comment?: string }>(req);
      if (!body.action || !['accept', 'reject'].includes(body.action)) {
        return json(res, 400, { error: 'action 必须是 accept/reject' });
      }
      try {
        const count = engine.resolveAllReviews(m[1], body.action as 'accept' | 'reject', { comment: body.comment ?? null });
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/human-task$/)) && method === 'POST') {
      const body = await readBody<{ taskId?: string; content?: string; note?: string }>(req);
      if (!body.taskId) return json(res, 400, { error: 'taskId 必填' });
      try {
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/artifacts\/restore$/)) && method === 'POST') {
      const body = await readBody<{ nodeId?: string; targetVersion?: number }>(req);
      if (!body.nodeId) return json(res, 400, { error: 'nodeId 必填' });
      if (typeof body.targetVersion !== 'number') return json(res, 400, { error: 'targetVersion 必填（数字）' });
      try {
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/artifacts\/diff$/)) && method === 'POST') {
      const body = await readBody<{ nodeId?: string; fromVersion?: number; toVersion?: number }>(req);
      if (!body.nodeId) return json(res, 400, { error: 'nodeId 必填' });
      const state = engine.getExecution(m[1]);
      if (!state) return json(res, 404, { error: 'execution not found' });
      ...
      if (!from) return json(res, 404, { error: `v${body.fromVersion} 不存在` });
      if (!to) return json(res, 404, { error: `v${body.toVersion} 不存在` });
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/artifacts\/code-diff$/)) && method === 'POST') {
      ...
      if (!chain.length) return json(res, 404, { error: `节点 ${body.nodeId} 无 code Artifact` });
      ...
      if (!from || !to) return json(res, 404, { error: 'code 版本不足，无法对比' });
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/artifacts\/code-file$/)) && method === 'POST') {
      const body = await readBody<{ nodeId?: string; version?: number; path?: string }>(req);
      if (!body.nodeId || !body.path) return json(res, 400, { error: 'nodeId 与 path 必填' });
      const state = engine.getExecution(m[1]);
      if (!state) return json(res, 404, { error: 'execution not found' });
      const chain = (state.artifacts[body.nodeId] ?? []).filter(a => Array.isArray(a.files));
      const art = chain.find(a => a.version === body.version) ?? chain.at(-1);
      if (!art) return json(res, 404, { error: 'code Artifact 不存在' });
      try {
    ...
    if ((m = p.match(/^\/api\/executions\/([^/]+)\/artifacts\/code-edit$/)) && method === 'POST') {
      const body = await readBody<{ nodeId?: string; files?: Array<{ path: string; content: string; language?: string }>; comment?: string }>(req);
      if (!body.nodeId) return json(res, 400, { error: 'nodeId 必填' });
      if (!Array.isArray(body.files) || !body.files.length) return json(res, 400, { error: 'files 必填（非空数组）' });
      try {
  ...
  function sse(res: ServerResponse, executionId: string | null): void {
    res.writeHead(200, {
    ...
    const send = (e: ExecutionEvent) => {
      if (executionId && e.executionId !== executionId) return;
  ...
  return {
    ...
    close: () => new Promise(resolve => {
      for (const unsub of liveStates.values()) unsub();
      liveStates.clear();
      server.close(() => resolve());
    }),
    async renderLanding(): Promise<string> {
...
function readBody<T>(req: IncomingMessage, limit = 8 * 1024 * 1024): Promise<T> {
...
function reqClose(res: ServerResponse, fn: () => void): void {
  res.on('close', fn);
}

// ---------------------------------------------------------------- cordis 入口

export function apply(ctx: any, rawConfig: any = {}) {
  const config = {
  ...
  if (!tryMountWorkflowEntry()) {
    ...
    const timer = setInterval(() => {
      ...
      if (tryMountWorkflowEntry() || attempts >= 5) {
        clearInterval(timer);
        if (attempts >= 5) ctx.logger?.warn?.('宿主未提供 webServer 注册能力，仅保留独立端口入口');
      }
    }, 500);
    timer.unref?.();
  }
