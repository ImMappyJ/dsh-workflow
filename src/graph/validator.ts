...
export function validateWorkflow(def: WorkflowDefinition): ValidationResult {
  ...
  if (starts.length === 0) errors.push({ code: 'missing_start', message: '缺少 start 节点' });
  if (starts.length > 1) errors.push({ code: 'multiple_start', message: 'start 节点只能有一个' });
  if (ends.length === 0) errors.push({ code: 'missing_end', message: '缺少 end 节点' });
  if (ends.length > 1) {
  ...
  for (const n of def.nodes) {
    if (n.type === 'agent') {
      if (!n.identity?.name) errors.push({ code: 'missing_identity', message: `节点 ${n.id} 缺少 identity.name`, nodeId: n.id });
      if (!n.roleDescription) warnings.push({ code: 'missing_role', message: `节点 ${n.id} 未填写角色描述`, nodeId: n.id });
  ...
  if (starts.length === 1) {
    ...
    if (!hasOut) errors.push({ code: 'start_no_out', message: 'start 节点没有出边' });
  }
  for (const end of ends) {
    const hasIn = (def.edges ?? []).some(e => e.target.nodeId === end.id);
    if (!hasIn) warnings.push({ code: 'end_no_in', message: `end 节点 ${end.id} 没有入边`, nodeId: end.id });
  }
...
function reachableFrom(start: string | undefined, adj: Map<string, string[]>, all: Set<string>): Set<string> {
  const seen = new Set<string>();
  if (!start || !all.has(start)) return seen;
  const queue = [start];
  while (queue.length) {
    const cur = queue.pop()!;
    ...
    for (const next of adj.get(cur) ?? []) {
      if (all.has(next) && !seen.has(next)) queue.push(next);
    }
  }
  return seen;
}
...
export function loopsFromSccs(sccs: string[][], existing: WorkflowDefinition['loops'], defaultMaxIterations = DEFAULTS.maxIterations) {
