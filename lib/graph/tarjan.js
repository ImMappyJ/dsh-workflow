 ...
 */
export function tarjanScc(nodeIds, adj) {
    let index = 0;
 ...
 */
export function detectLoops(nodeIds, adj) {
    return tarjanScc(nodeIds, adj).filter(scc => scc.length > 1 || (adj.get(scc[0]) ?? []).includes(scc[0]));
}
