 ...
 */
export function normalizeWorkspaceCwd(workspaceDir) {
...
export function extractDelta(event) {
    ...
    if (chunk.type === 'text-delta' && typeof chunk.text === 'string')
        return { text: chunk.text, kind: 'text' };
    if (chunk.type === 'reasoning-delta' && typeof chunk.text === 'string')
        return { text: chunk.text, kind: 'reasoning' };
    if (chunk.type !== 'delta')
        return { text: '', kind: 'text' };
    ...
    if (typeof chunk.delta?.text === 'string')
        return { text: chunk.delta.text, kind: 'text' };
    return { text: '', kind: 'text' };
}
/** 旧名兼容：只取文本内容 */
...
function unwrap(resp) {
    ...
    if (resp?.result?.value !== undefined)
        return resp.result.value;
    if (resp?.ok === false)
        throw new Error(resp.error?.message ?? 'apiProxy rpc failed');
    return resp;
}
// ---------------------------------------------------------------- AsyncQueue
...
// ---------------------------------------------------------------- provider
export class DshSessionProvider {
    api;
    opts;
    ...
    async *run(req) {
        ...
        if (!sessionId) {
            ...
            if (!sessionId)
                throw new Error('sessions.create 未返回 sessionId');
            isNewSession = true;
        }
        else {
            ...
            try {
                ...
                const accepted = unwrap(testResp);
        ...
        try {
            ...
            if (!accepted.accepted)
                throw new Error('sessions.prompt 未被接受');
            ...
            for await (const ev of queue) {
                if (ev.type === 'assistant/chunk') {
                    ...
                    if (d.text)
                        yield { kind: 'delta', text: d.text, deltaKind: d.kind };
                }
