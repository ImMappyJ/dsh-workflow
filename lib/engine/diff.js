...
export function lineDiff(oldText, newText) {
    if (oldText === newText)
        return { ops: [], added: 0, removed: 0, degraded: false };
    ...
    if (a.length > MAX_LINES || b.length > MAX_LINES) {
        ...
        return {
            ops: [
                ...a.map((t, i) => ({ type: 'del', oldLine: i + 1, text: t })),
                ...b.map((t, i) => ({ type: 'add', newLine: i + 1, text: t })),
            ],
            added: b.length,
            removed: a.length,
            degraded: true,
...
export function renderUnifiedDiff(result, maxLines = 40) {
    ...
    if (result.ops.length > maxLines)
        lines.push(`…（共 ${result.ops.length} 行，已截断）`);
    if (result.degraded)
        lines.unshift('!! 文本过大，已降级为整段替换展示');
    return lines.join('\n');
}
