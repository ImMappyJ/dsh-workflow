...
export function createReviewTask(state, executionId, edge, artifactId) {
 ...
 */
export function resolveReview(state, taskId, action, opts = {}) {
    const task = state.reviewTasks.find(t => t.id === taskId);
    if (!task)
        throw new Error(`review task ${taskId} 不存在`);
    if (task.status !== 'pending')
        throw new Error(`review task ${taskId} 已处理（${task.status}）`);
    ...
    if (action === 'accept') {
        ...
        if (latest && opts.comment)
            latest.reviewComment = opts.comment;
    ...
    if (action === 'edit' || action === 'accept_after_edit') {
        if (typeof opts.content !== 'string')
            throw new Error('edit 必须提供 content');
    ...
    if (action === 'reject') {
        if (!opts.comment)
            throw new Error('reject 必须提供审核意见（Reject Reason，§61）');
 ...
 */
export function createNodeReviewTask(state, executionId, nodeId, artifactId, reviewCfg) {
    const chain = state.artifacts[nodeId] ?? [];
    const task = {
        id: `review_${randomUUID().slice(0, 8)}`,
        executionId,
        edgeId: '__node__', // 标记为 node-level review
        sourceNodeId: nodeId,
        targetNodeIds: [],
        artifactId,
        artifactIds: chain.map(a => a.id).includes(artifactId)
