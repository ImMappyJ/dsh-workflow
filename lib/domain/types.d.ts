...
/** 输入字段类型谱系（任务 5） */
export type InputFieldType = 'text' | 'number' | 'boolean' | 'select' | 'file' | 'files' | 'directory' | 'json' | 'artifact';
 ...
 */
export declare function validateInputFields(schema: InputSchema | undefined, fields: Record<string, unknown> | undefined): string[];
export type WorkflowStatus = 'running' | 'paused' | 'waiting_review' | 'waiting_human' | 'completed' | 'failed' | 'terminated' | 'cancelled';
export type NodeStatus = 'idle' | 'queued' | 'running' | 'waiting' | 'waiting_review' | 'waiting_human' | 'success' | 'failed' | 'skipped' | 'cancelled';
...
export type ArtifactKind = 'text' | 'markdown' | 'json' | 'code' | 'file' | 'directory' | 'office' | 'image';
...
export declare function kindFromLegacyType(t: Artifact['type']): ArtifactKind;
export declare function legacyTypeFromKind(k: ArtifactKind): Artifact['type'];
/** outputContract.format → kind（引擎登记 Artifact 用） */
export declare function kindFromFormat(format: string | undefined): ArtifactKind;
...
export interface ExecutionState {
    ...
    pendingFeedback: Record<NodeId, HumanIntervention & {
        text: string;
        reviewId: string;
    }>;
...
export type WorkflowEventType = 'workflow.started' | 'workflow.completed' | 'workflow.failed' | 'workflow.terminated' | 'human_task.requested' | 'human_task.completed' | 'artifact.restored' | 'artifact.edited' | 'node.queued' | 'node.started' | 'node.input_received' | 'node.thinking' | 'node.output_generated' | 'node.completed' | 'node.failed' | 'node.skipped' | 'node.retrying' | 'edge.triggered' | 'edge.skipped' | 'loop.iteration' | 'loop.terminated' | 'execution.paused' | 'execution.resumed' | 'review.requested' | 'review.resolved' | 'review.edited' | 'review.timeout';
...
export declare const DEFAULTS: {
    readonly maxExecutionSteps: 100;
    readonly MAX_EXECUTION_STEPS_HARD_LIMIT: 1000;
    readonly defaultNodeMaxRuns: 5;
    readonly maxIterations: 3;
    readonly timeoutMs: 120000;
    readonly maxInputCharsPerSource: 8000;
    readonly maxTotalInputChars: 24000;
    readonly defaultPort: 3090;
};
export declare function defaultRuntimeConfig(partial?: Partial<RuntimeConfig>): RuntimeConfig;
export declare function defaultSettings(partial?: Partial<WorkflowSettings>): WorkflowSettings;
/** §4：旧 Definition 无 revision 时按 1 处理（向后兼容） */
export declare function effectiveRevision(def: Pick<WorkflowDefinition, 'revision'>): number;
/** §20：边路由缺省值——auto/bezier/无控制点 */
export declare function effectiveRouting(edge: WorkflowEdge): EdgeRouting;
